...
use skyrim_modmgr::{
    ...
    fomod,
    game::{
        find_skyrim_at, find_skyrim_at_smart, scan_all_locations, DeepScanOptions, GameInstall,
    },
    ini, nxm,
    profile::Profile,
    store::{self, ModStore},
...
#[derive(Subcommand)]
enum Commands {
    /// Scan for a Skyrim install (Steam/Proton/Lutris/Bottles/Heroic/…
    /// prefixes, common folders, and mounted drives). If more than one is
    /// found, you'll be asked which one to use.
    ///
    /// Use `--path` to point at an install root (or its `Data` folder)
    /// manually. Use `--deep` when the normal scan misses a non-standard /
    /// portable / relocated copy — it walks more of the disk and every
    /// Wine prefix it can find. Use `--from DIR` to deep-scan only certain
    /// trees (can be repeated).
    DetectGame {
        /// Install directory, or the game's `Data` folder.
        #[arg(long)]
        path: Option<PathBuf>,
        #[arg(long)]
        my_games_dir: Option<PathBuf>,
        /// Aggressive nested search across home, mounts, and free-floating
        /// Wine prefixes (slower; use when the default scan finds nothing).
        #[arg(long)]
        deep: bool,
        /// Extra root(s) to search (nested). Can be passed multiple times.
        #[arg(long = "from")]
        from: Vec<PathBuf>,
        /// Max directory depth for `--deep` / `--from` (default: 12).
        #[arg(long)]
        depth: Option<usize>,
    },
...
fn scan_and_pick_game(
    ...
    let found = scan_all_locations(extra_search_paths);
    if found.is_empty() {
        color::warn(
            "No Skyrim install found in the quick scan (known Wine/Proton/\
             PortProton/Lutris/Bottles/Heroic/CrossOver prefixes, Downloads/\
             Games/Desktop, mounts, custom search paths).\n\
             Try one of:\n\
               skyrim-modmgr detect-game --deep\n\
               skyrim-modmgr detect-game --from /path/to/search\n\
               skyrim-modmgr detect-game --path /path/to/SkyrimSE  (or its Data folder)",
        );
        return Ok(None);
    }
    pick_from_detected(found)
}

fn deep_scan_and_pick(
    from: &[PathBuf],
    depth: usize,
    extra_search_paths: &[PathBuf],
) -> Result<Option<skyrim_modmgr::game::DetectedGame>> {
    color::info(
        "Deep-scanning for Skyrim (nested folders + Wine prefixes). \
         This can take a minute on large disks…",
    );
    let mut roots = from.to_vec();
    roots.extend(extra_search_paths.iter().cloned());
    let opts = DeepScanOptions {
        roots,
        max_depth: depth,
        include_prefixes: true,
    };
    let found = skyrim_modmgr::game::deep_scan_for_skyrim(&opts);
    if found.is_empty() {
        color::warn(
            "Deep scan still found nothing. Point at the folder that contains \
             SkyrimSE.exe / TESV.exe / SkyrimVR.exe, or the game's Data folder \
             (must include Skyrim.esm):\n\
               skyrim-modmgr detect-game --path /full/path/to/install",
        );
        return Ok(None);
    }
    pick_from_detected(found)
...
fn pick_from_detected(
    mut found: Vec<skyrim_modmgr::game::DetectedGame>,
) -> Result<Option<skyrim_modmgr::game::DetectedGame>> {
    if found.len() == 1 {
        return Ok(Some(found.remove(0)));
    }

    println!("Found {} Skyrim install(s). Which one do you use?\n", found.len());
    ...
    match cli.command {
        Commands::DetectGame {
            ...
            if let Some(path) = path {
                let game = if let Some(my_games) = my_games_dir {
                    find_skyrim_at(&path, &my_games)
                } else {
                    find_skyrim_at_smart(&path)
                };
                match game {
                    Some(g) => {
                        color::success(&format!(
                    ...
                    None => color::warn(&format!(
                        "No Skyrim install at {} — need SkyrimSE.exe / TESV.exe / \
                         SkyrimVR.exe / skse*_loader, or a Data folder with Skyrim.esm \
                         (you can pass the Data folder itself).",
                        path.display()
                    )),
                }
            } else if deep || !from.is_empty() {
                let depth = depth.unwrap_or(12);
                if let Some(chosen) =
                    deep_scan_and_pick(&from, depth, &config.extra_search_paths)?
                {
                    color::success(&format!(
                        "Using {:?} at {} ({})",
                        chosen.game.edition,
    ...
    if let Some(install_dir) = install_dir {
        ...
        return find_skyrim_at(&install_dir, &my_games).context(
            "could not confirm a Skyrim install at the given path \
             (looking for SkyrimSE.exe / TESV.exe / SkyrimVR.exe / Data/Skyrim.esm)",
        );
    }
