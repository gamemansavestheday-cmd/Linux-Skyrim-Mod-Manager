...
use skyrim_modmgr::{
    ...
    config::Config,
    game::{
        deep_scan_for_skyrim, find_skyrim_at_smart, scan_all_locations, DeepScanOptions,
        DetectedGame,
    },
    profile::Profile,
    saves::{self, SaveFile},
...
impl App {
    ...
    /// Apply results of Detect / Deep scan: 0 → message, 1 → remember, many → picker.
    fn apply_detect_results(&mut self) {
        match self.detected.len() {
            0 => {
                self.status = "No Skyrim install found. Try Deep scan, browse for the \
                     install folder, or deep-scan a specific drive."
                    .to_string();
            }
            1 => {
            ...
            _ => {
                self.status = format!(
                    "Found {} installs — pick one in the window.",
                    self.detected.len()
                );
...
impl App {
    ...
    fn draw_deploy_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "Detection looks for Steam/Proton, Lutris, Bottles, Heroic, PortProton, \
             plain Wine, common folders, and mounted drives. It accepts official \
             exes, SKSE loaders, or a Data folder with Skyrim.esm — so non-Steam \
             / portable copies work too.",
        );
        ui.horizontal(|ui| {
            if ui.button("Detect game install(s)").clicked() {
                self.status = "Scanning…".to_string();
                self.detected = scan_all_locations(&self.config.extra_search_paths);
                self.apply_detect_results();
            }
            if ui
                .button("Deep scan…")
                .on_hover_text(
                    "Slower: walks home, mounts, and free-floating Wine prefixes nested. \
                     Use when Detect finds nothing.",
                )
                .clicked()
            {
                self.status = "Deep scanning (this may take a while)…".to_string();
                // Force a repaint so the status shows before we block.
                ui.ctx().request_repaint();
                self.detected = deep_scan_for_skyrim(&DeepScanOptions {
                    roots: self.config.extra_search_paths.clone(),
                    max_depth: 12,
            ...
            if ui
                .button("Browse for install…")
                .on_hover_text(
                    "Manually pick the game folder (the one with SkyrimSE.exe / Data) \
                     or the Data folder itself.",
                )
                .clicked()
            {
                if let Some(path) = rfd::FileDialog::new()
                    .set_title("Select Skyrim install folder (or its Data folder)")
                    .pick_folder()
                {
                    match find_skyrim_at_smart(&path) {
                        Some(game) => {
                            self.status = format!(
                                "Using {:?} at {}",
                                game.edition,
                            ...
                            self.config.remember_game(game);
                            let _ = self.config.save(&self.paths);
                            self.saves_loaded = false;
                        }
                        None => {
                            self.status = format!(
                                "Not a Skyrim install: {} — need an exe or Data/Skyrim.esm",
                                path.display()
                            );
                        }
                    }
        ...
        if ui
            .button("Deep-scan a specific folder…")
            .on_hover_text("Pick a root (e.g. an external drive) and search nested folders.")
            .clicked()
        {
            if let Some(root) = rfd::FileDialog::new()
                .set_title("Folder to search for Skyrim")
                .pick_folder()
            {
                self.status = format!("Deep scanning {}…", root.display());
                ui.ctx().request_repaint();
                self.detected = deep_scan_for_skyrim(&DeepScanOptions {
                    roots: vec![root],
                    max_depth: 14,
